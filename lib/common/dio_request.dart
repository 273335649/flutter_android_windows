...
class Request {
  ...
  static Future<void> init() async {
    await Constant.init();
    _dio.options.baseUrl = Constant.getBaseUrl();
    // 配置代理
  ...
  static Future _request(
    ...
    try {
      ...
      if (path.startsWith('/dev')) {
        // 本地代理到hf开发
        // print('ENV:${Constant.ENV}');
        _dio.options.baseUrl = Constant.getBaseUrl();
        requestPath = path.substring(4); // 去掉/dev前缀
      } else {
        _dio.options.baseUrl = Constant.domainUrl; //如果切换环境请改非空地址
      }
      ...
      Map<String, dynamic> dataObjec = {};
      if (e.response?.data is Map) {
        dataObjec = e.response?.data as Map<String, dynamic>;
      }
      var dataCode = dataObjec['code']?.toString() ?? "";
      if (errorCode?.statusCode == 401) {
        List logintCode = ["400", "40105", "40104", "40100", "40111", "40114"];
        print(
          '错误401 接口：${e.requestOptions.uri.toString()} 状态：${logintCode.contains(dataCode)} 数据代码：$dataCode headers: ${e.requestOptions.headers ?? 'null'} noConfer: ${e.requestOptions.headers?['noConfer'] ?? 'null'} 参数: ${e.requestOptions.data ?? 'null'}',
        );
